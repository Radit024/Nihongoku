...
router.post("/classroom/create", async (req, res) => {
  try {
    ...
    if (!user || user.role !== "dosen") {
      res.status(403).json({ error: "Hanya dosen yang dapat membuat kode kelas" });
      return;
    }
    ...
    const [updated] = await db
      .update(usersTable)
      .set({ classCode: nextCode })
      .where(eq(usersTable.id, user.id))
      .returning();

    res.json({ classCode: updated.classCode });
  } catch (err) {
...
router.post("/classroom/join", async (req, res) => {
  try {
    ...
    if (user.role !== "mahasiswa") {
      res.status(403).json({ error: "Hanya mahasiswa yang dapat bergabung kelas" });
      return;
    }
    ...
    const [classOwner] = await db
      .select({ id: usersTable.id })
      .from(usersTable)
      .where(and(eq(usersTable.classCode, classCode), eq(usersTable.role, "dosen")))
      .limit(1);
